...
export default function TimelineCanvas() {
    ...
    const [isDragging, setIsDragging] = useState(null); // taskId or null
    const [isResizing, setIsResizing] = useState(null); // taskId or null
    const [dragStart, setDragStart] = useState({ x: 0, startWeek: 0 });
    ...
    const handleMouseMove = (e) => {
        ...
        if (isDragging) {
            ...
            const newStartWeek = Math.max(
                0,
                Math.min(totalUnits - 1, dragStart.startWeek + deltaWeeks)
            );
    ...
    return (
        <div className="flex-1 overflow-auto" ref={canvasRef}>
            <div
                className="relative"
                style={{ width: `${totalUnits * weekWidth}px`, minHeight: "100%" }}
            >
                {/* Timeline headers */}
